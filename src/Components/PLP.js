...
const PLP = () => {
  ...
  // If you want to initialize products once, you can dispatch here
  useEffect(() => {
    // Only dispatch if products are empty
    if (!products || products.length === 0) {
      dispatch(setProducts()); // Assuming setProducts() fetches products or sets default products
    }
  }, [dispatch, products]);
          ...
          products.map((product, index) => (
            <div className="col" key={index}>
              <div className="card" style={{ width: "18rem" }}>
                <img
                  style={{ height: "250px" }}
                  src={product.Image} // Make sure your Redux state uses 'Image' not 'image'
                ...
                <div className="card-body">
                  ...
                  <p className="card-text">{product.description}</p>
                  <Link to="/pdp" state={product} className="btn btn-primary">
                    Read More
                  </Link>
                </div>
              </div>
